...
### Socket Timeouts (SO_RCVTIMEO / SO_SNDTIMEO)

Set on the Unix socket in `prepareYproxyConnection()`:
- `SO_RCVTIMEO` and `SO_SNDTIMEO` set to `yproxy_socket_timeout` seconds (default: 120).
- Acts as a safety net: if yproxy stops responding, the socket call returns with
  `errno = EAGAIN/EWOULDBLOCK` after the timeout.
...
The implemented approach combines two mechanisms:

**Layer 1 — SO_RCVTIMEO/SO_SNDTIMEO (safety net)**:
Socket-level timeouts prevent infinite hangs even if the poll() loop has a bug.
Minimal code, zero risk, standard POSIX.

**Layer 2 — poll() + InterruptPending check (primary mechanism)**:
